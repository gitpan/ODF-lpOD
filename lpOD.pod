...
method tells the C<odf_document> instance that the current part comes from that
it should be updated at the next C<save()>. The example below shows the way to
commit and write back the changes made in the C<META> and C<CONTENT> parts of a
previously loaded C<odf_document>:
