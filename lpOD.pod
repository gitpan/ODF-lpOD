...
ODF::lpOD - Languages & platform for OpenDocument - DEVELOPMENT VERSION
...
=head1	SYNOPSIS
        ...
        my $content = $document->get_content;
...
The code example shows above loads a document from an existing "report.odt" file,
...
I<The present distribution is a early developer version. It implements only
a small part of the lpOD functional specification. However it's reasonably
usable. The documented features will be extended and improved, but should not
be removed in the stable version.> 
...
=head3  odf_new_document_from_type(document_type)
...
C<'presentation'>, and C<'drawing'>). Example:
        ...
        my $document = odf_new_document_from_type('spreadsheet');
...
has been created from a template (i.e. through C<odf_new_document_from_type>
or C<odf_new_document_from_template>).
...
Note that this document-based method saves only the changes that had been
committed at the document level (see below).
...
I<The present development release covers C<'content'> and C<'meta only'>.
However, the common, low level odf_element methods work with other parts as
well.>

The odf_document class provides a set of C<get_part> methods, where C<'part'> may
be replaced by the name of the needed part, in order to provide access to every
individual part. Example:

        my $content = $document->get_content;
        my $meta = $document->get_meta;

The sequence above give access to the content and meta parts of a previously
created odf_document instance.

Beware: if the same get_part method is called twice or more from the same
odf_document instance, it returns the same object. As a consequence, after the
sequence below, C<$p1> and C<$p2> will be synonyms:

        my $p1 = $document->get_content;
        my $p2 = $document->get_content;

In order to make persistent the changes made in a part, an explicit
serialization must be done. Two methods are provided to do so.
...
C<store()> commits the changes at the document level; in other words, this
method tells the odf_document instance the current part comes from that it
should be updated at the next C<save()>. The example below shows the way to
commit and write back the changes made in the C<'meta'> and C<'content'>
parts of a previously loaded C<odf_document>:

        my $document = odf_get_document($source);
        my $meta = $document->get_meta;
        my $content = $document->get_content;
        # some processing of meta elements
        # some processing of content elements
        $meta->store;
        $content->store;
        $document->save(target => $destination);

Like C<serialize()>, C<store()> allows the C<'pretty'> optional parameter.
...
as shown below:
        ...
        my $document = odf_get_document($source);
        my $context = $document->get_content->get_root;
...
style "dp1" (assuming we are using a presentation or drawing document):
        ...
        my $context = $document->get_content->get_body;
...
The detailed documentation is split into several manual chapters which are
not necessarily linked to Perl packages.

So the presently implemented features (or most of them) are described in the
following manual pages
For details about the main lpOD classes, look at the following manual pages,
that cover a large part of the presently implemented features (that is a small
part of the full lpOD scope):

=over

=item *

L<ODF::lpOD::Packages>: General ODF package handling and metadata.

=item *

L<ODF::lpOD::Elements>: Common features, available with any element.

=item *

L<ODF::lpOD::TextElements>: Basic text containers (paragraphs, headings), and
various elements that may take place in paragraphs (bookmarks, index marks,
bibliography marks).

=item *

L<ODF::lpOD::Tables>: Tables and their content.

=item *

L<ODF::lpOD::StructuredContainers>: High-level structures other than tables

=item *

L<ODF::lpOD::Utilities>: Common utlities provided by the Perl lpOD library

=back

You may have a look at L<http://docs.lpod-project.org> in order to know the
scope of the lpOD project and get an idea about the additional features that
