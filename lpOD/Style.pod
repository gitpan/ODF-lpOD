...
Among the creation option for style paragraphs, a C<master page> parameter
...
style using C<insert_style()>. An optional display name argument is allowed
...
C<space before>: the ident level, i.e. the space before the number for all
...
C<align>: to specifiy the table alignment scheme, with C<left>, C<right>,
...
The most necessary property is C<width>, wich may be an absolute width (i.e.
...
and/or B<text> properties, so its full definition mya require three distinct
areas.
...
namespace prefix as a "draw" property (i.e. the C<draw:blue> color correction
