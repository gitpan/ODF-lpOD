...
This manual page describes the C<odf_style> class and its specialized
derivatives.

C<odf_style> is an alias for C<ODF::lpOD::Style>.

A style object controls the formatting and/or layout properties of a family of
content objects. It's identified by its own name and its family.

The lpOD API to update the properties of existing styles, and to replicate the
features of existing styles. It allows the user to build new styles from
scratch as well.

The full styling of a graphically sophisticated document could require a lot
of complex code, because a lot of layout graphical rendering options are
available. As a consequence, it's recommended to reuse and customize existing
styles each time it's possible, instead of hardcoding fully new styles, in
order to maintain a reasonable ratio between the style related and the content
related coding efforts.
...
C<odf_document> method, introduced below.

Additional named parameters may be required according to the family. An optional
...
The register a new style in a document, the generic C<insert_element()> and
C<append_element()> method are not appropriate. The document-based
C<insert_style()> method, that partly hides the complex ODF style storage
logic, must be used instead:

   $doc->insert_style($s);

C<insert_style()> requires a style object as its only one mandatory argument.
It's return value is the style itself.
...
Note that the C<register()> method of C<odf_style> allows the same optional
parameters as the C<insert_style()> or C<register_style()> document-based
method, provided that these parameters come after the mandatory argument that
...
A style may be created "in place" by C<insert_style()> if the first
...
family. So the following example sets a default paragraph style whose alignment
rule is C<'justify'>:
...
Such a default style definition means that, in the document, every paragraph
without specified style will be justified.
...
Knowing that a style may be C<automatic> or not, and that it may be stored
in the C<CONTENT> or C<STYLE> part of a document, C<insert_style()> silently
selects the options that work in most typical situations according to the
style family. However, it's sometimes useful or necessary to store a style
in a specific, non default way. To do so, C<insert_style()> (and its
derivatives) allows the following additional options:

=over

=item *

C<automatic>: boolean; if C<TRUE>, the style is stored as automatic, if
C<FALSE>, it's stored as common, and if C<undef>, it's stored according to
the lpOD's choice (that depends on the style family);

=item *

C<part>: may be C<CONTENT> or C<STYLE> (note that as soon as the storage part
is C<CONTENT>, the style will be I<automatic>, whatever the value of the
C<automatic> parameter, if any).

=back

These two last options are ignored if C<default> is C<TRUE>.
...
A C<set_shadow()> method is available; its options are the same as the
C<set_shadow()> method for graphic styles.
...
The C<set_background()> method is allowed with C<color> or C<url> (while C<url>
may be replaced by C<image>). Note that for this kind of style the C<url> or
C<image> is not a standard link but a symbolic resource name depending on
the viewer. More generally, C<set_background()> works with cell styles in the
same way as with graphic styles.

I<Caution>: The most used ODF presentation software tools don't handle tables
and table-related styles in the same way as ODF text or spreadsheet processors.
If C<set_background()> is executed when the cell style object is not registered
in a document, lpOD doesn't know the type of the target document, so it selects
a default behavior that is appropriate for text and spreadsheet documents. If
the target document type is a presentation, the user should ensure that the
style is registered before calling C<set_background()>. Alternatively, it's
possible to use the C<fill()> method, that works for presentation table cells.
...
C<'red'>, C<'green'>, and/or C<'blue'> specify a color correction parameter,
provided as a positive or negative percentage (a trailing percent sign should
appear in the given value);
...
A lot of other parameters are allowed; to get a full list, the user shoud have
a look at the I<Graphic styles> chapter in the public ODF specification. Note
that many ot them are C<draw> options, i.e. their names begin with the C<draw>
prefix; so if an option is set without prefix, lpOD generally regard it as
a C<draw> option. Among the exceptions, the C<wrap> and C<run through> options
correspond to ODF attributes whose prefix is C<style>, while C<clip> stands for
C<fo:clip>; lpOD can handle these exceptions, so the prefix is not required
with these options, too.

The C<set_background()> method may be used in the same way as with other style
families. Example:
...
A graphic style supports optional I<shadow> parameters. Thanks to the
C<set_shadow()> method, the most used ones may be easily handled. This
method may be used with the following options:

=over

=item

C<color> specifies the color of the shadow; the default is black;

=item

C<offset> controls the horizontal and vertical offset of the shadow; these
two values may be provided either as a comma-separated string or as a string
list reference; each one may be positive ot negative and should include a
trailing length unit string; the default is C<['3mm', '3mm']>, giving a
bottom-right 3mm shadow. Starting from the previous example, the following
instruction adds a light green shadow whose origin point is located 4mm west
and 2mm south from the origin point of the related object.
...
may be used through several I<master pages>.
