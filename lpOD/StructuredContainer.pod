...
true, means that I<if the numbering style of the preceding list is the same as
...
C<add_item()> wich creates one or more new items and inserts them at a
...
optional C<style> parameter, whose value is a regular paragraph style, may
...
resource is a readable image file in the local filesystem, and if the image
...
with C<set_title()>, the first argumentis the text of the title, and a
