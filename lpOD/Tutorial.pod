...
However, remember that lpOD can't automatically detect the original image
size unless the image file is not local; so, as soon as you associate a frame
to an external content you should provide an explicit size.
