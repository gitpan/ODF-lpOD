...
However, the so-called C<uri> must be a path in the local file system.
...
so, you must use the document-based C<insert_style()> method (don't use
C<insert_element()> for styles). The following instruction creates the new
paragraph style and includes it in the document:
