...
of the lpOD toolset through simple examples that execute various operations
...
Note that our code examples use Perl 5.10 syntax, so don't forget
C<use 5.010.xxx> if you use them unchanged (and remember that lpOD requires
Perl 5.10 or later).
...
C<odf_document> instance through the C<odf_get_document()> contructor with the
path/filename as the only mandatory argument:

        my $doc = odf_get_document("olddoc.odt")
                or die "Failed to load the document";

In a real application, don't forget to check the returned value. If something
...
In usual applications, you may have to select a particular workspace in the
document. The workspaces are called I<parts> here. The most commonly used are
represented by lpOD symbolic constants: C<CONTENT>, C<STYLES>, C<META>, and
...
object that appear in the page bodies:

        my $context = $doc->get_body;

This shortcut, which selects the I<body> element of the document content, is a
synonym of:
...
belongs to the C<odf_element> class and may be the context for a large set of
operations regarding its own features or its sub-elements (so-called
...
The last instruction above directly returns a particular subspace of the
C<CONTENT> document part, so-called the I<"document body">. This subspace
contains or can contain the various objects that populate the document (text
paragraphs, tables, images, lists, and so on). We can use it as the
I<"context"> for subsequent operations regarding such objects.
...
flag is C<FALSE>. Of course this flag may be reset to C<TRUE> through at any
time through a subsequent use of C<needs_update()>.
...
An office document owns a set of so-called "metadata". Metadata is "data about
the document". For the end user, it may be got (and sometimes changed)
through the "File/Properties" submenu of a typical desktop software. lpOD
...
I<modification date>, the I<creator>, and others. All this data belongs to the
C<META> document part. We can get access to any of them through the C<META>
context. Note that the C<META> document part is directly usable as the context
for metadata access, so we don't need to look for a particular I<body> element
in this part:
...
if uses the name of the system user of the current process (provided that Perl
can get such an information from the operating system).
...
A piece of metadata whose datatype is I<date> should be locale-neutral; it's
...
In the C<META> context, you can use or change the document keywords.
...
following examples exports the keywords as a concatenated string, then in
separate lines:
...
that whose metadata includes a keyword provided through the command line:
...
and C<set_user_fields()>. The user defined metadata looks like a 3-column table
whose properties are I<name>, I<value>, and I<type>.

C<get_user_fields()> returns a list whose each item is 3-element a hash ref
containing these informations. So, the example below displays the full set of
...
discover some features that are not text-specific and that introduces more
general aspects of the lpOD element management logic.

Beware: the present chapter is not excusively about I<text documents>. In
...
The most simple recipe is the "Hello World" example that creates and insert
...
Without explicit explicit option, the given object is inserted as the first
child of the context, so the new paragraph will appear at the very beginning of
our document (whatever the existing content). We could use C<append_element()>,
that puts the new element at the end of the context. In addition,
C<insert_element()> can take optional parameters allowing the user to specify
a particular position, before or after another element, already existing in
the context.
...
be, defined elsewhere). We'll have a look to text styles in another recipe.
You can create text element without explicit style name as long as the default
presentation of your favourite viewing/editing software is convenient for you.
...
course in real applications we need more flexibility. Fortunately there is an
alternative use of C<insert_element()>. We can, for example, insert a new
paragraph immediately before or after another paragraph. It's possible thanks
to an alternative use of C<insert_element()>, that may be called from any
kind of element instead of the document body. In the next example, we select
a particular paragraph (say, the last one) then we insert a new paragraph
before it:

        my $p = $context->get_paragraph(position => -1);
        $p->insert_element(
                odf_create_paragraph(
                        text    => "We are before the last pararaph",
                        style   => "Standard"
                        ),
...
The most straightfoward is the position in the document sequence. The
...
directly or indirectly contain text paragraphs. W'll see some of these high
...
Remember that C<$p->isa(odf_paragraph)> is true when C<$p->isa(odf_heading)>
is true, because C<odf_heading> is a subclass of C<odf_paragraph>. So the order
of the tests can matter.
...
The generic style constructor is C<odf_create_style()>. It requires a style
family identifier as its first argument, knowing that, say, a table style
or an image style is not the same thing as a paragraph style. The style
...
Once created, the style must be included in the document style set. To do so,
you must use the document-based C<insert_style()> method (don't use
C<insert_element()> for styles). The following instruction creates the new
paragraph style and include it in the document:
...
Note that c<insert_style()> returns the inserted style, so the application
can keep it in order to invoke it later. The instruction above produces the
same effect as the sequence below:
...
facto standards) color codes are 6-digit hexadecimal values with a leading "#".
In this example the blue value is "80" while the red and green are zero.
...
chack or change its properties, to clone it and/or to export it from a
...
replacing 'paragraph' by 'text' as the family name:
...
paragraph text, whatever the style of this paragraph style and its possible
default text properties. This option requires the use of C<set_span()>. The
...
value may be a string or a regex) specifies that the given style must apply to
...
paragraph containing "ODF", like that:
        ...
        $p = $doc->get_body->get_paragraph(content => "ODF");
        $list = $p->insert_element(odf_create_list);
...
C<set_level_style()> with a C<style> option whose value should ne the name
...
On the other hand, if we create a level 2 sublist (i.e. a new list contained in
...
control more precisely the label spacing and aligment through extended list
...
If you need to create tables, we encourage you to use the GUI of your favourite
...
end. To do so, we just have to specify a position. The next insctruction
...
not for tables included in text documents.
...
the coordinates, so the two instuctions below are equivalent whatever the
...
We could can get or set the text content of a table cell using C<get_text()>
or C<set_text()>. But doing so we just use the cell as a text container, while
...
This introductive tutorial is not focused on performances. However some code
...
th the next table row if there is no cell at the current C<$j> position. Why
...
The systematic use of C<set_properties()> in this example could be ommitted,
...
links each columns to its style:
...
blue border with a 1.25mm grey shadow:
        ...
        $cs->set_background(color => "#ffff00");
...
issue for any application that build tables from scratch or that imports
tables from other documents, because a single table may directly or indirectly
depend on dozens of styles. However, lpOD provides you with some facilites for
individual or bulk style importation (see L<ODF::lpOD::Style>).

=head1  Inserting frames, images and boxes

A frame is a generic rectangular are that may contain, for example, an image
or a simple or complex text element.
...
local filesystem, and try the following example (where you can replace the
given filename by the full path of your real image file). In this example,
we assume that C<$doc> is a I<text> document.

        $context = $doc->get_body;
        $p = $context->append_element(odf_create_paragraph);
        $p->append_element(
                odf_create_image_frame("logo.png")
                );
...
We have probably introduced the simplest way to insert an frame; it consists
...
The way the coordinates are interpeted depends on some properties of the
...
image should be properly embedded in the document as long as you lauch you
...
the C<add_file()> document-based method, that allows you to store almost
anything in your office document files. Example:

        $doc->add_file("logo.png");
...
of C<add_file()> and use it as the image resource identifier in place of the
file name:

        $link = $doc->add_file("logo.png");
        $doc->get_body->append_element(
                odf_create_image_frame(
                        $link,
                        style           => "Classic",
...
Note that we introduced the text box creation with a litteral text content
...
litterals and/or ODF text elements (such as paragraphs) as arguments and
...
This first example, assuming that the calling object is a previously selected
paragraph, the following instruction inserts a the filename at the default
...
The thing may turn more complicated as soon as you want to display formatted
...
Many fields may be linked to the same variable: their role may be to display
in several places a value stored once.
...
For other informations about the fields and variables, see
L<ODF::lpOD::TextElement>.
...
Then, say, remove any existing draw page in order (just to show the way):
...
C<position> parameter apply is, of course, relative to the area of the
draw page. While the explicit use of a graphic style is not always required
by every presentation viewer, it's recommended to provide it (or, as a
propably better option, to reuse an external style library). The creation
and registration of a graphic style works exactly like with text documents.

Of course, you can/must include the image file in your ODF package using
C<add_file()> as previously shown with another kind of document.
...
existing page, change some details (including og course the name and the
...
And, if we know nothing but the sequential position in presentation order,
