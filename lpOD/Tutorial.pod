...
as is as the only argument of a subsequent call C<odf_create_element()>
...
However, for any reason, you may need to put on-utf8 content in documents.
...
However we'll not always know a recognized color name for every color (while
a few hundreds are defined).
...
Not that C<get_table()> is available as a shortcut 
...
presentation viewer, it's recommended to provide it (or, as a propably better
