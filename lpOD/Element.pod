...
If the given string starts with C<'http:'>, then it's regarded as the URL of
an XML resource available through the web. If L<LWP::Simple> is installed and
if the remote resource is available, then this resource is parsed as XML.
...
The same principle applies for every subclass of C<ODF::lpOD::Element>; as a
...
the imported elements are clones of the original ones, the source contexte is
...
methods specifically described below, programmers familiar with C<XML::Twig>
...
is overriden by some specialized element classes.
