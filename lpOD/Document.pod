...
Same as C<add_file()> but, in array context, returns the destination path and
(provided that C<Image::File> is installed and the image format is supported)
the image size. This size (if defined) may be used to set the size of the
corresponding image container in the document (see the "Frames" section in
