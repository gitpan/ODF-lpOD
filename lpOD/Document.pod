...
the MIME type using C<File::Type>. If C<File::Type> is not installed of doesn't
recognize the file format, lpOD doesn't provide any default value, so the mime
type of the resource is not registered in the document. Note that right MIME
types are not absolutely required by typical ODF-compatible software but that
it's a good practice to provide them when possible.
...
section in L<ODF::lpOD::StructuredContainer>).

However, the returned image size is significant only if C<Image::Size> is
installed and if the image format is recognized by this module (fortunately,
the most popular formats, such as PNG, JPG, BMP, XPM, TIF and others are
supported).
...
If C<File::Type> is not installed or if it fails, then lpOD arbitrarily
...
that may hopefully provide a correct MIME type in some situations. And if nothing works
(i.e. if there is no application-provided type, if C<File::Type> doesn't
answer, and it there is no file suffix), then the type is set to
...
coorectly render an image in a document (provided that the image format is
